  ...
  try {
    ...
    if (action === "send") {
      ...
      if (typeof leadId === "string" && leadId.trim()) {
        ...
        const contractStatus = await getLocalLiftContractStatus(leadId.trim());
        if (!contractStatus.ok) {
          return res.status(502).json({ success: false, reason: "contract_status_unavailable", error: contractStatus.error || "No se pudo verificar el contrato antes de enviar." });
        }
        if (!contractStatus.signed) {
          return res.status(409).json({ success: false, reason: "contract_not_signed", contractStatus: contractStatus.status || "sent", error: "El cliente todavía debe revisar y firmar el contrato desde su portal." });
        }
