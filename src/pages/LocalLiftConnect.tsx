...
export default function LocalLiftConnect() {
  ...
  useDocumentTitle("Guía de implementación Ascenso | Polaris", "Ascenso implementation guide | Polaris", "", "");
      ...
      <main className="max-w-lg mx-auto px-4 sm:px-6 py-16 md:py-24">
        ...
        {status === "notincluded" && (
          <div className="text-center rounded-[var(--radius-bento)] glass-panel p-10 border border-[var(--color-border-subtle)]">
            ...
            <p className="mt-4 text-sm leading-relaxed text-[var(--color-text-secondary)]"><T en="This guide is available for Ascenso clients. Impulso includes prepared content for you to apply yourself.">Esta guía está disponible para clientes Ascenso. Impulso incluye el contenido preparado para que tú lo apliques.</T></p>
          </div>
        )}

        {status === "ready" && (
          <div className="rounded-[var(--radius-bento)] glass-panel p-8 border border-[var(--color-primary-base)]/20">
            <p className="text-xs font-black uppercase tracking-[0.2em] text-[var(--color-primary-base)]"><T en="Local Lift · Ascenso">Local Lift · Ascenso</T></p>
            ...
            <p className="mt-5 text-sm leading-relaxed text-[var(--color-text-secondary)]">
              <T en="Ascenso does not require you to connect your Google account to Polaris. We prepare the content, show you where each change goes and guide you step by step.">
