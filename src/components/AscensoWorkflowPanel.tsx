...
const HISTORY_EVENT_LABEL: Record<string, string> = {
  ...
  package_ready_for_approval: "Paquete listo para revisión",
  package_sent: "Paquete enviado al portal",
    ...
    <section className="rounded-[var(--radius-bento)] border border-indigo-500/20 bg-indigo-500/[0.04] p-4 sm:p-6">
      <details className="group">
        ...
        <div className="mt-6 space-y-6">
          <div className="rounded-xl border border-teal-500/25 bg-teal-500/[0.06] p-4"><p className="text-[10px] font-black uppercase tracking-widest text-teal-700">Contenido del paquete · borrador para revisión</p><p className="mt-1 text-xs leading-relaxed text-[var(--color-text-secondary)]">Este contenido coincide con el PDF de revisión que puedes descargar arriba. Revisa aquí y en el PDF los textos, publicaciones y respuestas antes de solicitar cambios o aprobarlo.</p></div>
      ...
      {data?.packageSent && packageData && (
        <div className="space-y-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-widest text-[var(--color-primary-base)]">Guía de implementación · documento separado</p><h3 className="mt-1 text-base font-display font-black text-[var(--color-text-primary)]">Cómo aplicar el contenido del paquete</h3><p className="mt-1 text-xs leading-relaxed text-[var(--color-text-secondary)]">Esta sección explica dónde y cómo aplicar lo que aparece en el paquete. La guía PDF completa se habilitará por separado con la entrega final; las solicitudes de cambios corresponden al paquete, no a esta guía.</p></div>
          </div>

          <div className="rounded-lg border border-indigo-300/50 bg-indigo-500/10 p-4 text-xs leading-relaxed text-[var(--color-text-primary)]"><strong className="text-indigo-700">Cómo usar este documento:</strong> abre una sección, reúne lo que necesitas antes de empezar, sigue las instrucciones y registra tus dudas desde el portal. Los nombres y botones pueden variar según tu cuenta; si algo no aparece, detente y consúltalo con Polaris. Las rondas sirven para ajustar el material preparado; no significan que Polaris publique por ti.</div>
          <div className="flex flex-wrap gap-2 text-[11px] font-bold text-[var(--color-text-primary)]"><span className="rounded-full border border-[var(--color-border-subtle)] bg-[var(--color-surface-highlight)] px-3 py-1">{guideSteps.length} secciones</span><span className="rounded-full border border-[var(--color-border-subtle)] bg-[var(--color-surface-highlight)] px-3 py-1">{packageData.googlePosts?.length || 0} publicaciones</span><span className="rounded-full border border-[var(--color-border-subtle)] bg-[var(--color-surface-highlight)] px-3 py-1">{packageData.reviewReplies?.length || 0} respuestas</span><span className="rounded-full border border-[var(--color-border-subtle)] bg-[var(--color-surface-highlight)] px-3 py-1">{packageData.services?.length || 0} servicios</span></div>
            ...
            {guideSteps.map((step: any, index: number) => (
              <details key={`${step.title}-${index}`} open={index === 0} className="group rounded-xl border border-[var(--color-border-subtle)] bg-[var(--color-surface-base)]/40 p-4">
                ...
                <div className="mt-4 space-y-4 border-t border-[var(--color-border-subtle)] pt-4">
                  ...
                  <div className="rounded-lg border border-amber-400/45 bg-amber-500/10 p-3"><p className="text-[10px] font-black uppercase tracking-widest text-amber-700">Evita estos errores</p><ul className="mt-2 list-disc space-y-1 pl-4 text-xs leading-relaxed text-[var(--color-text-primary)]">{(step.avoid || []).map((item: string, itemIndex: number) => <li key={itemIndex}>{item}</li>)}</ul></div>
