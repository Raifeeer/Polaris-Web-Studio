...
const STATUS_LABEL: Record<string, string> = {
  awaiting_connection: "Paquete enviado",
  awaiting_client_review: "Paquete enviado · listo para revisar",
  changes_requested: "Solicitud pendiente",
  revision_ready: "Revisión enviada al cliente",
  pending_admin_review: "Cliente aprobó · acompañamiento listo",
...
export default function AscensoAdminWorkflowPanel({ leadId, token, packageSnapshot, onWorkflowChange }: Props) {
  ...
  const mutate = async (action: string, extra: Record<string, unknown> = {}) => {
    ...
    try {
      ...
      onWorkflowChange?.(result.workflow);
      setNotice(action === "admin_send_revision" ? "La revisión se envió al cliente." : action === "admin_mark_published" ? "El acompañamiento quedó marcado como completado." : "El servicio fue cerrado.");
    } catch (e: any) {
  ...
  const hasPackageSnapshot = !!packageSnapshot;

  return (
    <section className="rounded-xl border border-indigo-500/25 bg-indigo-500/[0.04] p-5 space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="flex items-start gap-3"><div className="w-9 h-9 rounded-lg bg-indigo-500/10 flex items-center justify-center"><ShieldCheck size={17} className="text-indigo-400" /></div><div><p className="text-[10px] font-black uppercase tracking-widest text-indigo-400">Gestión Ascenso</p><h3 className="mt-1 text-base font-black text-[var(--color-text-primary)]">Revisión y acompañamiento</h3><p className="mt-1 text-[11px] text-[var(--color-text-secondary)]">Prepara cada revisión, explica al cliente cómo aplicar los cambios y acompáñalo durante el proceso. Este servicio no publica directamente en Google.</p></div></div><span className="rounded-full border border-indigo-500/30 bg-indigo-500/10 px-2.5 py-1 text-[10px] font-black uppercase tracking-wider text-indigo-300">{STATUS_LABEL[workflow.status] || workflow.status}</span></div>
      ...
      <div className="space-y-3">
        ...
        {workflow.requests.slice().reverse().map((request: any) => (
          <div key={request.id} className="rounded-lg border border-[var(--color-border-subtle)] bg-[var(--color-surface-base)]/35 p-4 space-y-3"><div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><Clock size={13} className="text-amber-400" /><span className="text-xs font-black">Ronda {request.round} de {request.maxRounds}</span></div><span className="text-[10px] uppercase font-black tracking-wider text-[var(--color-text-tertiary)]">{request.status}</span></div><p className="whitespace-pre-wrap text-xs leading-relaxed text-[var(--color-text-secondary)]">{request.requestText}</p>{["submitted", "in_progress", "changes_requested"].includes(request.status) && <div className="space-y-2"><div className="rounded-lg border border-indigo-500/20 bg-indigo-500/[0.06] p-3 text-[11px] leading-relaxed text-[var(--color-text-secondary)]"><strong className="text-indigo-300">Cómo preparar esta ronda:</strong> modifica el paquete en las secciones de arriba y guarda cada cambio. El botón siguiente toma esa versión actual, regenera el PDF marcado como borrador y lo envía al portal del cliente. No uses “Enviar paquete completo”.</div><textarea value={responseDrafts[request.id] || ""} onChange={(e) => setResponseDrafts((previous) => ({ ...previous, [request.id]: e.target.value }))} rows={4} placeholder="Escribe qué cambiaste y qué debe revisar el cliente…" className="w-full rounded-lg border border-[var(--color-border-subtle)] bg-[var(--color-surface-base)] p-3 text-xs outline-none focus:border-indigo-400" /><button type="button" disabled={!responseDrafts[request.id]?.trim() || !hasPackageSnapshot || !!busy} onClick={() => void mutate("admin_send_revision", { requestId: request.id, adminResponse: responseDrafts[request.id], packageSnapshot })} className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-xs font-black text-white disabled:opacity-50"><Send size={13} /> Generar y enviar revisión</button>{!hasPackageSnapshot && <p className="text-[11px] font-bold text-amber-300">Carga o genera el paquete actual arriba antes de enviar la revisión.</p>}</div>}{request.adminResponse && <div className="rounded-lg border border-indigo-500/20 bg-indigo-500/5 p-3"><p className="text-[10px] font-black uppercase tracking-widest text-indigo-400">Respuesta enviada</p><p className="mt-1 whitespace-pre-wrap text-xs text-[var(--color-text-secondary)]">{request.adminResponse}</p></div>}</div>
        ))}
      </div>

      {workflow.status === "pending_admin_review" && <div className="space-y-2"><p className="text-[11px] leading-relaxed text-[var(--color-text-tertiary)]">Cuando hayas completado el acompañamiento con el cliente, confirma aquí para cerrar el ciclo y actualizar su portal.</p><button type="button" disabled={!!busy} onClick={() => void mutate("admin_mark_published")} className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2.5 text-xs font-black text-white disabled:opacity-50"><CheckCircle2 size={15} /> Confirmar acompañamiento completado</button></div>}
